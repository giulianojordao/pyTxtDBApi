...
"""
class Row :
	_id = 0   			#// unique id for the row
	_fields = []		#// fields of the row
...
class ResultSet :
	...
	************************************/
	...
	#// rows 
	_rows = []  #// to use as array of type Row (see above)
	...
	_orderColNrs = [] 	#// Column Nr
	_orderTypes = []   	#// ORDER_ASC or ORDER_DESC
	...
	************************************/
	"""
	def setRowId(self, _rowNr, _id) :
		self.rows[_rowNr].id = _id
	#enddef
	def getRowId(self, _rowNr) :
		return self.rows[_rowNr].id
	#enddef
	def setCurrentRowId(self, _id) :
		self.rows[self.pos].id = _id
	#enddef
	def getCurrentRowId() :
		return self.rows[self.pos].id
	#enddef
	def searchRowById(self,_id) :
		for _i in range(self.rows) : 
			if(self.rows[_i].id != None and self.rows[_i].id == _id) :
				return _i
		#endfor
		return NOT_FOUND;
	#enddef
	...
	def generateRowIds() :
		...
		while(++self.pos < len(self.rows)) :
			self.rows[self.pos].id = ++ _rId
		#endwhile
		-- self.pos
	#enddef
	
	def setAllRowIds(_self, _val) :
		self.reset()
		while(++ self.pos < len(self.rows)) :
			self.rows[self.pos].id = _val
		#endwhile
		-- self.pos
	#enddef
